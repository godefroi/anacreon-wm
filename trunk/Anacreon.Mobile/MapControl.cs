...
namespace Anacreon.Mobile
{
	public partial class MapControl : UserControl
	{
		...
		{
			var sect_width  = Universe.Sectors.GetLength(0);
			var sect_height = Universe.Sectors.GetLength(1);
			var char_width  = (sect_width * 3) - 2;
			var skip_x      = 15 - ((Universe.HomeSector.X * 3) % 15);
			var skip_y      = 5 - (Universe.HomeSector.Y % 5);

			using( var g = Graphics.FromImage(m_curmap) )
			{
				g.Clear(Color.Black);

				// draw each line
				for( var y = 0; y < sect_height; y++ )
				{
					var x = 2;

					IEnumerable<char> grid_enum;

					if( skip_y++ % 5 == 0 )
						grid_enum = GenerateGridRow();
					else
						grid_enum = GenerateNormalRow();

					// draw the actual row
					foreach( var thingy in Enumerate(grid_enum.Skip(skip_x).Take(char_width), GenerateNebulaRow(Universe.Sectors, y), GenerateObjectRow(Universe.Sectors, y, Brushes)) )
					{
						var brush = Brushes.Gray;
						var mchar = thingy.Value1;
						...
						if( thingy.Value3.Character != '\0' )
						{
							// object sector
							brush = thingy.Value3.Brush;
							mchar = thingy.Value3.Character;
						}

						g.DrawString(mchar.ToString(), Font, brush, x++ * CharacterSize.Width, (y + 1) * CharacterSize.Height);
					}
				}
			}
		}*/
		...
		/*private static IEnumerable<char> GenerateNebulaRow(Sector[,] sectors, int row)
		{
			var sect_count = sectors.GetLength(0);

			//MessageBox.Show(sect_count.ToString());

			if( sectors[0, row].Nebula )
			{
				yield return "▒";
				yield return "▒";
			}
			...
			for( var x = 1; x < sect_count - 1; x++ )
			{
				if( sectors[x, row].Nebula )
				{
					yield return "▒";
					yield return "▒";
					yield return "▒";
			...
			if( sectors[sect_count - 1, row].Nebula )
			{
				yield return "▒";
				yield return "▒";
			}
		...
		/*private static IEnumerable<SectorChar> GenerateObjectRow(Sector[,] sectors, int row, Brushes brushes)
		{
			var sect_count = sectors.GetLength(0);

			yield return GetMapRepresentation(sectors[0, row], brushes);
			yield return SectorChar.Empty;

			for( var x = 1; x < sect_count - 1; x++ )
			{
				yield return SectorChar.Empty;
				yield return GetMapRepresentation(sectors[x, row], brushes);
				yield return SectorChar.Empty;
			}

			yield return SectorChar.Empty;
			yield return GetMapRepresentation(sectors[sect_count - 1, row], brushes);
		}*/

		/*private static SectorChar GetMapRepresentation(Sector sector, Brushes brushes)
		{
			if( sector.Object != null )
			{
				if( sector.Nebula && !sector.Probed )
					return SectorChar.Empty;

				switch( sector.Object.Type )
				{
					case SpaceObjectType.Construction:
						switch( ((Construction)sector.Object).ConstructionType )
						{
							case ConstructionType.CommandBase: return new SectorChar('b', brushes.Gray);
							default: throw new Exception("Unknown construction type");
						}

					case SpaceObjectType.World:
						if( !sector.Probed )
							return new SectorChar('p', brushes.Red);

						switch( ((World)sector.Object).WorldType )
						{
							default: return new SectorChar("w", brushes.Gray);
						}
		...
		private static IEnumerable<Tuple<T1, T2, T3>> Enumerate<T1, T2, T3>(IEnumerable<T1> t1_enum, IEnumerable<T2> t2_enum, IEnumerable<T3> t3_enum)
		{
			var en_t1 = t1_enum.GetEnumerator();
			var en_t2 = t2_enum.GetEnumerator();
			var en_t3 = t3_enum.GetEnumerator();

			while( en_t1.MoveNext() && en_t2.MoveNext() && en_t3.MoveNext() )
				yield return new Tuple<T1, T2, T3>(en_t1.Current, en_t2.Current, en_t3.Current);
		}
